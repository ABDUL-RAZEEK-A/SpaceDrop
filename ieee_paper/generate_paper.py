...
<br><br>
Keywords—Local Area Network (LAN), Multicast DNS (mDNS), TCP Orchestration, HTTP Streaming, Flutter, File Distribution, Role-Based Access Control (RBAC), Edge Computing, Decentralized Systems, Peer-to-Peer Networks.
</div>
...
<div class="figure">Fig. 1. The three-layer separation between Discovery, Orchestration, and Data planes.</div>
...
<div class="equation">
$T_{cloud} \approx \frac{S}{B_{up}} + \frac{N \times S}{B_{down}}$
</div>
<p>For $S = 1\text{GB}$, $N = 30$, $B_{up} = B_{down} = 100\text{Mbps}$ ($\approx 12.5\text{MB/s}$):</p>
<div class="equation">
$T_{cloud} \approx 80\text{s} + 2400\text{s} = 2480\text{seconds (}\approx 41\text{ minutes)}$
</div>
...
<div class="equation">
$T_{space} \approx \frac{N \times S}{B_{loc}}$
</div>
<p>For $B_{loc} = 800\text{Mbps}$ ($\approx 100\text{MB/s}$):</p>
<div class="equation">
$T_{space} \approx \frac{30 \times 1\text{GB}}{100\text{MB/s}} = 300\text{seconds (5 minutes)}$
</div>
...
with open('ieee_paper/SpaceDrop_IEEE_Paper.html', 'w') as f:
